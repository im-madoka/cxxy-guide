...
在 C++11 标准中，引入了一种更简洁的循环语法：范围 for 循环。它能够自动遍历容器（如数组、vector 等）中的每一个元素，而不需要我们手动管理下标。
...
例如，我现在有一个数组 `arr`，我想要遍历其中的数据，我就可以这样来写：
...
当然，这个所谓 `容器` 不一定是一个数组。只要是拥有迭代器的类型，都可以放入这个位置，例如 `vector` 等等。具体的内容将会在后续章节中提到。

范围 for 循环在某些时候能让我们更加便捷的遍历容器中的元素。还是以上述例子来举例，如果没有范围 for 循环，我们需要这样写：

```cpp
int arr[] = {1, 2, 3, 4, 5};
for (int i = 0; i < 5 /* 这里假定你知道他的长度是 5 个，有时你甚至不知道他的长度是多少，可能还会更复杂 */; i ++) {
    std::cout << arr[i] << std::endl;
}
```

不过，范围 for 循环没法获取当前元素在这个迭代器里是第几个元素，所以如果你要精确定位元素，你还是得使用之前的 for 循环，或者手动添加一个定位变量。
